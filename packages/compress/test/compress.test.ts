...
describe("compress", () => {
  ...
  it("deterministic", () => {
    const src =
      "The database configuration is the implementation detail we need."
    expect(compress(src)).toBe(compress(src))
  })
  it("intensity ultra is stricter than lite", () => {
    const src =
      "The configuration is basically really important because of the implementation."
...
describe("redactPrivate", () => {
  it("strips balanced tags", () => {
    expect(redactPrivate("before <private>secret</private> after")).toBe(
      "before  after"
    )
  })
